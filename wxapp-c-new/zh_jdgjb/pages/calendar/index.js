...
Page({
  ...
  onLoad: function(options) {
    ...
    var _this = this;
    // 页面初始化 options为页面跳转所带来的参数
    var d_time = wx.getStorageSync("ROOM_SOURCE_DATE");
    console.log(d_time)
    var checkInDate;
    var checkOutDate;
    if (d_time.checkInDate || d_time.checkOutDate == '') {
      checkInDate = d_time.checkInDate;
      checkOutDate = d_time.checkOutDate;
    } else {
      checkInDate = options.checkInDate ? options.checkInDate : Moment(new Date()).format('YYYY-MM-DD');
      checkOutDate = options.checkOutDate ? options.checkOutDate : Moment(new Date()).add(1, 'day').format('YYYY-MM-DD');
    }
    wx.getSystemInfo({
    ...
    this.selectDataMarkLine(options.chenk)
  },
  ...
  //选择的入住与离店时间段
  selectDataMarkLine: function(index) {
  ...
  onPressDate: function(e) {
    ...
    if (!this.data.markcheckInDate) {
      ...
      // month = month + '月';
      // date = date + '日';
      // var strs = this.data.checkOutDate;
